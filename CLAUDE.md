...
Action versions are watched by `.github/dependabot.yml` (the `github-actions` ecosystem only; npm and Go modules are deliberately not watched there). It opens a weekly PR per group — `actions/*` in one, every third-party action in the other — and those PRs are deliberately **not** auto-labelled `ci`, because that label is the collaborator trust gate and a bot PR proposing an unread action version should not trip it. Label the first-party group to validate it. The third-party group mostly lands on `release-*.yml` and `benchmark-*.yml`, which no label reaches, so validating one of those means a `workflow_dispatch` dry run. Node is pinned to 22 at every site that names one — `release-traceway.yml`, `release-docs.yml`, `release-website.yml`, `benchmark-processor.yml`, the source-map upload recipe in the docs, the `node:22-alpine` Dockerfiles — matching the `>=22` floor in `frontend/package.json`. `node-version: latest` is used nowhere and should not come back: `docs/package.json` and `website/package.json` declare no floor, so it silently floated two production deploys onto whatever Node had shipped that morning.
