...
Action versions are watched by `.github/dependabot.yml` (the `github-actions` ecosystem only; npm and Go modules are deliberately not watched there). It opens a weekly PR per group — `actions/*` in one, every third-party action in the other — and deliberately does **not** auto-apply `ci`, so a bot PR passes the same collaborator gate as any other. Label the first-party group to validate it. The third-party group mostly lands on `release-*.yml` and `benchmark-*.yml`, which no label reaches, so validating one of those means a `workflow_dispatch` dry run — and note that labelling a PR which touches no gated workflow yields zero checks, which reads like a pass but is silence. Every updatable pin is a bare major tag, so Dependabot only ever proposes major bumps here.

Node is pinned per site rather than read from one file. `release-traceway.yml`, `release-docs.yml`, `release-website.yml` and `benchmark-processor.yml` each name `22` literally, because `frontend/package.json`'s `engines.node` is a floor (`>=22`) and `setup-node`'s `node-version-file` would resolve a floor to the newest release — so CI deliberately does not read it, while the `flake.nix` dev shell (above) still does. `node-version: latest` is used nowhere and should not come back: `docs/package.json` and `website/package.json` declare no floor at all, so it silently floated two production deploys onto Node Current. The `node:22-alpine` build stages (and `node:22-bookworm-slim` in `Dockerfile.browser`) are kept in step by hand — nothing asserts they agree with `engines.node`. The one remaining Node 20 reference is the `testing/devtesting-nestjs` fixture.
