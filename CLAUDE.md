...
#### Firebolt Telemetry Backend (ClickHouse-replacement candidate, opt-in)

Built with `-tags "transactional_pg telemetry_firebolt"` (pure Go, `CGO_ENABLED=0` fine): the **main DB is PostgreSQL** and the **telemetry DB is a self-managed Firebolt engine** spoken to over its HTTP SQL API — the same deployment shape as `transactional_pg telemetry_ch`, with Firebolt in ClickHouse's seat. Key facts:

- **Connection:** `backend/app/db/db_telemetry_firebolt.go` opens `db.TelemetryDB` (*sql.DB) via the official `firebolt-go-sdk` with DSN `firebolt:///<FIREBOLT_DATABASE>?url=<FIREBOLT_URL>` (env: `FIREBOLT_URL` default `http://localhost:3473`, `FIREBOLT_DATABASE` optional).
- **lit dialect:** the engine accepts `?` placeholders only. `db.Driver` is `lit.PostgreSQL` in this build (main DB), so the firebolt repo package (`backend/app/repositories/telemetry/firebolt/`) pins ALL its lit registrations and named-query rendering to `lit.SQLite` via its `registerModels`/`parseNamed` helpers — never use `db.Driver` for telemetry queries there. `db.TelemetryDriver` (`lit.SQLite`) exists for shared code (e.g. the retention worker) that renders named queries against `db.TelemetryDB`. The package also re-registers the shared `sqlitetypes` result models to the `?` dialect (lit's registry is last-write-wins per type and this package's init runs after sqlitetypes').
- **Writes:** chunked multi-row `INSERT INTO ... VALUES` through `insertRows()` (500 rows/statement); a failed statement fails the frame (500, SDK retries) like the SQLite/CH backends. Upserts use `ON CONFLICT (cols) DO UPDATE SET x = excluded.x` / `DO NOTHING` — supported and enforced by Firebolt's `UNIQUE(...)` table constraints. No inline `PRIMARY KEY` (rejected); `sessions` uses `UNIQUE(id)`.
- **Schema** (`backend/app/migrations/firebolt_telemetry/`): every table carries a `PRIMARY INDEX` matching its read pattern, and `0002_aggregating_indexes.up.sql` creates AGGREGATING INDEXes (Firebolt's flagship feature: auto-maintained mergeable aggregate states — including exact `PERCENTILE_CONT` states — with transparent query rewrite) for the dashboard queries. Rewrite rules (all verified by EXPLAIN against the live engine): a query merges index states only when everything it references is an index key, an expression over a key, a filter on a key, or an indexed aggregate; `FILTER (WHERE ...)` on an aggregate blocks the rewrite; any reference to the raw time column forces a full scan.
- **Index-served dashboard reads.** The hot aggregate queries are written to those rules: time keys are `DATE_TRUNC('minute', col)`, so query time filters use the `indexMinuteRange`/`bindMinuteRange`/`minuteRangeParams` helpers (windows snap to minute bounds — <60s edge shift, below dashboard resolution) and chart buckets use `indexBucketExpr` (epoch-floor over the minute key; grouping by an expression over a key still rewrites). The grouped-endpoints read is a two-half join (`queryGroupedEndpointsIndexed`): counts/Apdex half plus a percentile half that filters `is_stream = 0` via the index key, because `FILTER` on the percentile would block the rewrite. Per-endpoint slow-endpoint offsets can't live in the index (dynamic data), so the fixed-threshold index rows are corrected by `refineOffsetEndpoints` — a raw scan bounded to just the endpoints with configured offsets. Queries whose filters/aggregates fall outside the index keys (arbitrary tag filters, `MAX_BY` "last", body search, attribute filters) keep a raw-scan fallback path; raw-row listings and their pagination counts are always raw so totals match. The exceptions grouped view is not yet index-served (needs a representative-row fetch the index can't express).
- **Dialect gotchas vs DuckDB** (the repo package already encodes these): `quantile_cont(x,p)` -> `PERCENTILE_CONT(p) WITHIN GROUP (ORDER BY x)`; time buckets via `TO_TIMESTAMP(FLOOR(EXTRACT(EPOCH FROM col)/N)*N)` (`timeBucketExpr`); `json_extract_string` -> `JSON_POINTER_EXTRACT_TEXT` with RFC 6901 pointers (dynamic keys bound through `jsonPointerEscape`); `json_keys` -> `UNNEST(JSON_POINTER_EXTRACT_KEYS(col, ''))` (root pointer is the empty string); `arg_max` -> `MAX_BY`; `INSTR` -> `STRPOS`; `string_agg(DISTINCT ...)` unsupported -> `ARRAY_TO_STRING(ARRAY_DISTINCT(ARRAY_AGG(x)), ',')`; `CAST(... AS DOUBLE)` -> `DOUBLE PRECISION`.
- **Retention:** Firebolt has no TTLs; the embedded-store retention worker prunes it (gate is `db.TelemetryDB != nil`, i.e. any of SQLite/DuckDB/Firebolt; `SQLITE_RETENTION_DAYS` applies). VACUUM is automatic (engine `AUTO_VACUUM`, on by default).
- **Tests:** `testhelper_firebolt_test.go` runs the shared telemetry suite against a live engine when `TEST_FIREBOLT_URL` is set (skipped otherwise).
- **Bench mode `pgfb`** (`benchmarks/compose/docker-compose.pgfb.yml`, `Dockerfile.firebolt`): the pgch twin for the hardware benchmark; engine image override via `FIREBOLT_IMAGE`.
- **Engine operations (established empirically against the live engine):** background compaction (auto-vacuum) is not enabled by default in this engine build, and uncompacted insert-tablets degrade reads superlinearly as tables grow — every deployment MUST pass a server config enabling it: `--server-config` with a YAML whose root needs `schema_version: "1.0"` and `engine.auto_vacuum.enabled: true` (`benchmarks/compose/firebolt-config.yaml`; look for "Auto-VACUUM worker has started." in the log). The engine sizes its memory budget from the **cgroup limit** (verified: `-m 3g` -> "Available RAM: 3.00 GiB" at startup) — uncapped it takes the whole box and starves colocated services, so compose sets `mem_limit` (`FIREBOLT_MEM_LIMIT`, default 4g). Spilling, the disk cache, and VACUUM of large tables need **io_uring**, which Docker's default seccomp profile blocks — run with `--security-opt seccomp=unconfined` on Linux, as the engine's own startup warning advises (macOS Docker cannot provide it at all; VACUUM/spill fail there). The ingest hot path renders SQL literals instead of SDK `?` substitution (3.9x faster measured; quote-doubling is the only escaping — backslashes are literal). Insert writes are chunked 500-row multi-row INSERTs.
- **Production scaling note:** single-node local-disk (`--data-dir`) is the dev/bench shape. Firebolt's supported production self-managed path is the Kubernetes operator — engines over S3-compatible object storage behind a gateway with a `dedicated-pensieve` metadata service; local filesystem mode is explicitly unsupported there.
