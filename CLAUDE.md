...
SECRETS_KEY=                          # base64 of 32 random bytes (openssl rand -base64 32) for `app/secrets`, the AES-256-GCM envelope every stored credential goes through (notification channel tokens today; integration configs, repository credentials and agent profile keys as the fix agent lands). Ciphertexts are `v1:<keyid>:<base64>`, the key id being the first 8 hex chars of SHA-256 over the key, so `secrets.Rotate` can tell rows apart. Unset on a SQLite main DB: a key file `traceway.secrets.key` (0600) is generated next to the database on first boot with a logged warning, and `:memory:` databases get an ephemeral key. Unset on PostgreSQL: exits 1 with an actionable message. A wrong key surfaces as `secrets.ErrWrongKey` at the first decrypt, naming the variable. `backfill.RunSecretsEncryption` encrypts any plaintext channel credential on every boot (idempotent, advisory lock 824737005 on Postgres); `GET /api/notification-channels` masks credentials with `********` and lists them in `hasSecrets`, and an update that sends the sentinel back keeps the stored value (`notifications.KeepStoredSecrets`). `notifications.NewAdapter` is the one decrypt point, so the outbox snapshot rests encrypted and is decrypted at send time.
...
# Fix agent executor (backend/app/agentrunner; the control plane in backend/app/agent is always on)
AGENT_MODE=off                        # off | embedded | remote. embedded runs AGENT_WORKERS harness goroutines in the backend process claiming attempts from the queue (the :agent image, which carries Node, the Claude Code CLI, git and bubblewrap); remote leaves the queue to traceway-agent-runner binaries. off (default) keeps the control plane and history working, hides Traceway AI Agent from the Fix it menu, and blocks new dashboard starts. GET /api/agent/capabilities exposes enabled from AGENT_MODE; preflight uses configuration, not runner liveness.
AGENT_WORKERS=2                       # embedded mode: concurrent attempts, each with its own workspace and sandbox
AGENT_SANDBOX=auto                    # auto | bwrap | off, parsed like SYNTHETICS_BROWSER_SANDBOX by the shared backend/app/sandbox package (auto probes bubblewrap and degrades to off; explicit bwrap fails fast at startup). The agent process runs with a pid/ipc/uts/user namespace, a fresh /proc, the system dirs read-only, a minimal /etc, only its workspace writable, sibling workspaces hidden behind a tmpfs, and exactly the allowlisted environment. bwrap cannot filter egress: NetworkAllow degrades to a shared network there and is enforced by the docker backend on remote runners; cpu/memory/pid limits need systemd-run on the host and are otherwise logged once as unsupported.
AGENT_SKILL_DIR=                      # directory holding the traceway skill (SKILL.md) bound read-only into the agent's sandbox; the :agent image sets /opt/traceway/skill. Unset = the prompt still points the agent at the skill path, which then does not exist
AGENT_DEFAULT_IMAGE=                  # runner only: the docker sandbox image when the repository names none (traceway:agent works: it carries node, claude, git and the traceway CLI under /opt/traceway); AGENT_DOCKER_RUNTIME passes --runtime (runsc)
AGENT_SANDBOX_ALLOW_OFF=false         # embedded mode refuses AGENT_SANDBOX=off (or an auto that degraded to off) unless this is true; dev only
AGENT_RUNNER_SECRET=                  # shared bearer credential of the traceway-agent-runner fleet; required for AGENT_MODE=remote (fail-fast at startup). Like the synthetics runners: deployment infrastructure, self-registered liveness rows in agent_runners keyed by X-Traceway-Runner-Name (with the capabilities JSON the runner sends in X-Traceway-Runner-Capabilities), claim identity "runner:<name>", rotation = change + restart. Runner polls determine health/deep liveness (2 min), not menu visibility or preflight; remote mode accepts queued attempts even with no online runner. Runner-side env: TRACEWAY_URL, AGENT_RUNNER_SECRET, AGENT_RUNNER_NAME, AGENT_RUNNER_WORKERS (2, max 16), AGENT_RUNNER_STATE (mirrors + workspaces), AGENT_SANDBOX (auto|docker|bwrap|off; docker is runner-only), AGENT_SANDBOX_ALLOW_OFF, AGENT_DEFAULT_IMAGE, AGENT_DOCKER_RUNTIME, AGENT_SKILL_DIR, TRACEWAY_CLI, AGENT_RUNNER_MONITORING
...
AGENT_ATTEMPT_RETENTION_DAYS=90       # fix-agent attempts: after this many days a terminal attempt loses its event stream and its storage blobs (agent/<attemptId>/transcript.jsonl, diff.patch, report.md); the attempt row and its messages stay as the issue page's history. 0 disables. All modes (retention/agent_attempts.go)
...
frontend/
├── src/
│   ├── routes/              # SvelteKit pages (settings/+page.svelte hosts the Integrations and Agent profiles cards, routes/settings/integration*-*.svelte and agent-profile*-*.svelte)
│   ├── lib/
...
/agent                      Fix agent: TabsRow Attempts (status filter, pending-approval box with approve buttons) | Repository (the project's code host integration, owner/name/branch, sandbox image, setup + test commands; read-only for readonly members)
/agent/[id]                 Attempt run page: status/agent/model/cost/PR meta, Approve + Cancel (write role), Timeline (polls /events?after=seq every 2s until terminal, merged by seq), Diff/Report/Transcript blob tabs, Thread column with origin pills and a reply box (write role, active attempts only)
...
**Fix agent** (control plane in `backend/app/agent`; see the "Fix agent" section below). Harness (`backend/app/agentrunner`): `Runner` runs one claimed attempt through `Claimer` (claim, `Inputs`, git credential) and `Reporter` (transition, renew, events, finding, blob, `Finish` with `Outcome`), both in `control.go`; `Local` is the in-process implementation over the database that the embedded executor and the runner protocol's server side share, `remote.Client` the HTTP one the `traceway-agent-runner` binary uses. The pipeline pushes the fix branch and reports `fixed`; `Local.Finish` opens the draft PR, so no executor holds more than a push credential. `sandbox.Docker` (`sandbox/docker.go`) is the runner-only backend; `Runner.HostTools` decides whether the host's agent and CLI directories are bound (bwrap/off) or the image provides them (docker). User docs: `docs/pages/learn/agent/` (overview, fix-it, repositories, profiles, slack, environment, security) and the `:agent` image page `docs/pages/server/agent.mdx`; the compose example is `docker-compose.agent.yml`. The release workflow builds and signs `traceway:agent` from `Dockerfile.agent` next to the browser image
| Method | Endpoint | Auth | Purpose |
|--------|----------|------|---------|
| GET | `/api/agent/preflight?hash=` | App | The Fix it dialog's four checks (repository, agent profile, executor, conversation channel) with hints and links, the next attempt number, the active attempt if one exists, the org's profiles and `canStart` |
| POST | `/api/agent/attempts` | App+Write | Start an attempt for an exception hash (`{hash, profileId?}`): 201 with the new attempt, or 200 with `existing: true` and the active one; runs `AttemptLimitHook`; wakes the queue on commit |
| POST | `/api/agent/attempts/list` | App | Paginated attempts of the project (`{status, pagination}`) |
| GET | `/api/agent/attempts/by-subject?hash=` | App | Every attempt for an issue with its links (the issue page card) |
| GET | `/api/agent/attempts/badge` | App | `needsInput` and `pendingApproval` counts for the sidebar |
| GET | `/api/agent/attempts/:id` | App | Attempt + links + `latestSeq` |
| GET | `/api/agent/attempts/:id/events?after=` | App | Event stream after a sequence number (strictly increasing `seq`), plus the current status |
| GET/POST | `/api/agent/attempts/:id/messages` | App / App+Write | The thread after a message id / post a reply (inbound `web` message; resumes an attempt waiting for input or review, wakes the queue) |
| POST | `/api/agent/attempts/:id/cancel` | App+Write | Cancel from any active status; 409 once terminal |
| POST | `/api/agent/attempts/:id/approve` | App+Write | Release a `pending_approval` attempt; 409 otherwise |
| GET | `/api/agent/attempts/:id/diff`, `/report`, `/transcript` | App | The stored diff / report / transcript blob (`agent/<id>/diff.patch`, `report.md`, `transcript.jsonl`); 404 until the executor has written them |
| POST | `/api/agent/run-token` | Run token | Denied (403): sandbox tokens cannot renew themselves; only the current executor claim can renew through the runner protocol |
| GET | `/api/integrations?kind=` | App | Enabled integrations of the project's organization as `{id, name, provider, kinds}` (no config), for members picking one in a channel dialog |
| POST | `/api/agent-runners/poll` | Agent runner (shared secret) | Long-poll claim of queued attempts for `runner:<name>` (`{maxAttempts}`, 25s hold on the queue wake channel, remote mode only, no Transactional); the runner protocol's other routes are `GET attempts/:id/context` (the `agentrunner.Inputs` wire type: attempt, repository, clone URL, decrypted profile, rendered pack, run token, session id to resume), `POST attempts/:id/git-credential`, `POST attempts/:id/transition`, `POST attempts/:id/events` (batch append + lease renewal; empty batch = heartbeat; 409 `claim lost` once the lease is gone), `POST attempts/:id/findings`, `PUT attempts/:id/blobs/:name` (transcript.jsonl/diff.patch/report.md, `?append=1`), `POST attempts/:id/result` (`agentrunner.Outcome`; idempotent: a lost claim answers `applied:false`). Every handler acts through `agentrunner.Local{Executor: "runner:<name>"}`, the same control-plane code the embedded executor calls in-process, so the two executors cannot drift; `Local.Finish` opens the draft PR, posts the thread message and sets the final status, which is why a runner never holds more than the push credential |
| POST | `/api/integrations/:provider/inbound/:integrationId` | None (rate-limited) | Every provider's webhooks and callbacks. The provider verifies the signature with the integration's own secret; `agent.HandleInbound` first lets an `InboundResponder` answer a transport handshake (Slack's `url_verification` challenge comes back verbatim), then decodes through every port the provider implements and hands the result to `agent.Dispatch` |

**Operations.** `GET /api/health/deep` carries an `agent` block (`agent.HealthSnapshot`: queued / pending approval / needs input / active counts by status, runners online within `agent.RunnerOnlineWindow`, last reclaim, and process counters for started, finished by status, reclaimed and cost); `monitoring.StartAgentReporter` emits them as `traceway.agent.{queued,pending_approval,needs_input,active,runners_online}` gauges and `traceway.agent.{started,finished,finished.<status>,reclaimed,cost_usd}.delta` every 10s when monitoring is on, charted by the seeded `traceway-agent` dashboard template. Every credential minted for a run (the run token in `Local.Inputs`, the git credential in `Local.GitCredential`) is an `agent.EventCredential` event on the attempt. The runner protocol routes are rate limited per IP (1200/min). Operator guidance: `docs/pages/learn/agent/runbook.mdx`; the threat model is the last section of `docs/pages/learn/agent/security.mdx`.

**Rules and CI runs.** Notification channel type `agent` (`notifications.AgentChannelType`, config `{profileId?, approval: ask|auto}`, `ParseAgentChannelConfig`) is special-cased in `dispatch()` next to `escalation`: a fire whose message carries an exception hash (`DedupToken`) and `ProjectId` goes to the registered `AttemptStarter` (`agent.StartFromRule`, wired in `cmd/run.go`), which starts the attempt in its own transaction (origin link `rule:<id>`, provider `rule`), asks every chat integration implementing `agent.ApprovalRequester` for approval when pending (Slack posts a message with Approve/Dismiss buttons, `traceway_approve`/`traceway_dismiss` block actions gated by the member's write role), or wakes the queue; fires without a hash are recorded as `skipped`, the cooldown starts at the fire, and the channel's Test answers 422. `POST /api/agent/attempts/report` (App+Write, NOT under Transactional: it runs `agentrunner.Local{Executor: "ci"}.FinishWithPullRequest`, which opens its own transactions) lets the auto-fix GitHub Actions contract record a CI run as an attempt: `{hash, status: fixed|analysis, branch?, pullRequestUrl?, report}` creates (or takes over a queued/pending) attempt, walks it through the run states as executor `ci` and ends it with the PR link and the report; the CLI's `traceway agent report|attempts|show` (`cli/cmd/traceway/agent.go`, `pkg/client/agent.go`) and the publish action's `report_attempt` input use it.

**GitHub** (`backend/app/integrations/github`, provider `github`, kinds code_host + issue_tracker + chat + trigger). One integration row, two credential modes on the `mode` field: `pat` (a fine-grained token in `token`) or `app` (App id, client id/secret, PEM private key, webhook secret, installation id, plus the issue `label` that starts attempts, default `traceway`). App tokens: `app.go` signs a 9-minute RS256 App JWT with golang-jwt and mints installation tokens per repository (`contents`/`pull_requests`/`issues` write, `metadata` read), cached until a minute before expiry; `CloneCredential` carries the hour, the runner protocol's `git-credential` returns it. Manifest flow (`manifest.go` + `controllers/github_manifest.controller.go`): `POST /api/integrations/github/manifest` (org admin, body `{organizationId, integrationId?}`) signs a 15-minute state with `services.SignState` and returns the page to open; `GET .../manifest/start?state=` is the self-submitting form to `github.com/settings/apps/new`; `GET .../manifest/callback?code=&state=` exchanges the code (`/app-manifests/{code}/conversions`), stores the credentials encrypted on the named integration or a new `GitHub App <slug>` row, and redirects to the install page; the `installation` webhook records/clears `installationId`. The provider's `SetupFlow.URL` is the API-relative POST path the settings dialog calls, not a link. Webhooks land on the shared inbound route, verified with `X-Hub-Signature-256` (`inbound.go`, body restored per port): `ping`/`installation` are answered by `Respond`; `issues labeled` with the label, or `@traceway fix` in a comment, makes a `Request` when the issue body carries `Hash:` and `Project:` lines of their own (the prepare action's rule; the github notification adapter appends the `Project:` line from `NotificationMessage.ProjectId`), authored by the `identities(provider=github)` row of the sender's login (admin-entered under Settings > Team Members, `GET/PUT/DELETE /api/organizations/:organizationId/members/:userId/identities(/:provider)`, stored lowercase); `@traceway <text>` on the attempt's PR or origin issue is an `InboundMessage` on that `pr`/`thread` link; `pull_request closed` is a `CodeHostEvent` that moves the attempt to `merged` (and archives the exception through the telemetry repository, `agent.archiveSubject`) or `closed`. As a channel `Open` returns the origin issue as the thread and `Post` comments; `agent.Mirror` also delivers to `pr` links, so once `Local.Finish` opens the PR the thread continues there. `Describe` feeds the repository tab's code host status (`codeHostView.status`). The `github` notification channel accepts an optional `integrationId` (token from the integration through `notifications.RegisterGitHubTokenSource`).

**Slack** (`backend/app/integrations/slack`, provider `slack`, kinds chat + trigger). One integration row is one Slack app in one workspace: fields `botToken` (xoxb-), `signingSecret`, optional `appToken` (xapp-, turns on Socket Mode) and `channel` (the default channel id), the three secrets encrypted at rest. The manifest to create the app from is `integrations/slack/manifest.yaml` (documented at `docs/pages/learn/agent/slack.mdx`). Two transports, one handler: the HTTP route above (v0 signature, 5-minute window, body restored so every port can read it) and Socket Mode (`socketmode.go`: `Manager` started from `cmd/run.go`, one `RunContext` loop per enabled integration with an app token, reconciled every 30s and on `agent.IntegrationsChanged()`, which the integration controller fires after commit; a changed token restarts the loop, disable/delete stops it). Both reduce Slack's payloads to one `envelope` (`envelope.go`) and run `requests`/`replies` (`handle.go`). Triggers: the `Fix it` button on an alert (value `{projectId, hash}`, origin link kind `alert` = `<channel>/<ts>`, so the thread opens under the alert), `@Traceway fix <issue url or hash>` (origin `origin` under the mention) and `/traceway fix <ref>` (thread as a new message in the command's channel); a bare hash is looked up in every project of the org and must be unique. The `Archive` button archives the issue. Identity: `identities(provider=slack)` by Slack user id, created on first contact from the workspace email via `users.info` matched against `users.email`; unmapped members, readonly roles and mistakes get a threaded (or ephemeral, for slash commands) reply and nothing else happens. Channel: `Open` posts the attempt's first message (under the origin message or in the default channel) and records the `thread` link `<channel>/<thread_ts>`; `Post` mirrors thread entries (the agent's by kind, other surfaces as `_Name via web_`); `Inbound` accepts messages in threads the app opened from linked members, deduplicated on `<channel>/<ts>`. Notification channel type `slack_app` (`adapter.go`, registered through `notifications.RegisterAdapter`, config `{integrationId, channel}`) posts Block Kit alerts with View (absolute URL only), Fix it and Archive (only when the message names an issue: `NotificationMessage.ProjectId` is set at dispatch, the hash comes from the URL); the controller checks `notifications.IntegrationBound` adapters against the project's organization. The old incoming-webhook `slack` type is untouched. `agent.Dispatch` opens the requesting channel's thread outside the start transaction (`openThread`) and records the link afterwards.
| GET | `/api/agent-profiles` | App | The project's org profiles (preflight picker; credential never returned, `hasCredential` instead) |
| GET/POST | `/api/organizations/:organizationId/agent-profiles` | Member / Admin | List / create profiles (`agent` must be `claude-code` for now; the first profile becomes the default) |
| PUT/DELETE | `/api/organizations/:organizationId/agent-profiles/:id` | Admin | Update (credential: blank or `********` keeps the stored one) / delete |
| POST | `/api/organizations/:organizationId/agent-profiles/:id/default` | Admin | Make it the org default (one at a time) |
| GET/PUT/DELETE | `/api/repositories` | App / +Write / +Write | The project's one repository binding (`integrationId`, owner, name, branch, image, setup and test commands) plus the org's code host integrations to pick from |
| GET | `/api/integrations/providers` | App | Every registered provider with its kinds, `Fields()` and setup flow: the settings page renders the form from it |
| GET/POST | `/api/organizations/:organizationId/integrations` | Admin | List (secret fields masked with `********` + `hasSecrets`) / create (`{provider, name, config}`, validated by the provider, secrets encrypted) |
| PUT/DELETE | `/api/organizations/:organizationId/integrations/:id` | Admin | Update (sentinel keeps a stored secret; `enabled` toggle) / delete |
| POST | `/api/agent-runners/poll` | Agent runner (shared secret) | Long-poll claim of queued attempts for `runner:<name>` (`{maxAttempts}`, 25s hold on the queue wake channel, remote mode only, no Transactional); the runner protocol's other routes are `GET attempts/:id/context` (the `agentrunner.Inputs` wire type: attempt, repository, clone URL, decrypted profile, rendered pack, run token, session id to resume), `POST attempts/:id/git-credential`, `POST attempts/:id/transition`, `POST attempts/:id/events` (batch append + lease renewal; empty batch = heartbeat; 409 `claim lost` once the lease is gone), `POST attempts/:id/findings`, `PUT attempts/:id/blobs/:name` (transcript.jsonl/diff.patch/report.md, `?append=1`), `POST attempts/:id/result` (`agentrunner.Outcome`; idempotent: a lost claim answers `applied:false`). Every handler acts through `agentrunner.Local{Executor: "runner:<name>"}`, the same control-plane code the embedded executor calls in-process, so the two executors cannot drift; `Local.Finish` opens the draft PR, posts the thread message and sets the final status, which is why a runner never holds more than the push credential |
| POST | `/api/integrations/:provider/inbound/:integrationId` | None (rate-limited) | The one inbound route for every provider's webhooks; the provider verifies the signature, `agent.HandleInbound` routes requests, replies and code host events |
...
| `integrations` | One row per connected provider per org for the fix agent (Slack, GitHub, ...): provider, `kinds` JSON, name, encrypted `config`, enabled; unique `(organization_id, provider, name)` |
| `identities` | External account (Slack member, GitHub login) to Traceway user mapping; unique `(provider, external_id)`; the one authorization rule for channels and triggers |
| `repositories` | Project to code repository binding (one per project): `integration_id` of the code host, owner, name, default branch, sandbox image, setup and test commands |
| `agent_profiles` | Org-scoped coding-agent configuration: agent, package, model, provider, base URL, encrypted `credential`, turn/timeout/budget limits, `allowed_tools` and `network_policy` JSON, one default per org |
| `agent_attempts` | One fix attempt per row (UUID id): org, project, repository, profile, per-subject `number`, `subject_kind`/`subject_ref` (an exception hash), executor, status, lease (`claimed_by`, `lease_expires_at`), branches, agent/model, cost and tokens, `report_key`. Partial unique index on `(project_id, subject_kind, subject_ref)` while the status is active |
| `agent_attempt_events` | Append-only sequenced event stream per attempt (`seq` unique per attempt); pruned by `AGENT_ATTEMPT_RETENTION_DAYS` for terminal attempts |
| `agent_messages` | The attempt's conversation thread: direction in/out, provider (web, slack, github), link and identity, kind, body |
| `agent_links` | Every external artifact of an attempt (origin, thread, issue, pr, alert); unique `(provider, kind, external_ref)` |
| `project_telemetry_sources` | Which `integrations` row answers a telemetry domain for a project, with priority (Traceway is implicit) |
| `agent_runners` | Liveness and capabilities of remote agent runners, self-registered by unique name like `synthetic_runners` |
...
## Fix agent (Stage 1 of tracewayapp/traceway#299)

**Security/lifecycle contract (2026-09-05):** `agent.StartAttempt` is the shared subject/project authorization and quota boundary; its `StartLimitHook` forwards the legacy controller hook. Inbound links use integration + active attempt scope, and Slack external identities are prefixed with integration ID. Every claim receives a new opaque `claimed_by` value; `executor` remains the runner label. Remote per-attempt calls carry `X-Traceway-Claim`; `Local.claimed` atomically renews/checks ownership before state, events, artifacts, credentials and outcomes. Preflight reuses its transaction for remote liveness. Default chat opening is persisted with starts and replayed through the outbox, whose remote delivery is at-least-once.

The harness exports source files into `workspace.Snapshot`, with trusted Git metadata outside writable mounts; it never publishes through agent-controlled `.git`. Tests run on the snapshot and a mutated patch is refused. New branches are `traceway/fix-<attempt UUID>`; resumes retain their branch and PR. GitHub reconciles creation by head/base, and publication holds the claim write lock through the bounded provider request. Run tokens use the CLI's `run` credential kind and are atomically renewed before expiry; renewal failure cancels execution. Output is bounded/redacted, usage accumulates across completed sessions, and Claude receives the remaining cooperative CLI budget. Agent/test resource requests are 2 CPUs, 2048 MiB and 256 PIDs (Docker enforced, bwrap only with systemd support). Docker allowlisting refuses unsupported hosts, filters both DOCKER-USER and INPUT, and overrides image ENTRYPOINT. Containerized Docker runners require the host network namespace, NET_ADMIN and identical absolute state paths; prefer a native Linux runner. Bwrap still has no egress filter. These corrections supersede the earlier implementation summary below.

The backend is the control plane for fix attempts and never calls a model. `backend/app/agent` holds the attempt state machine, the queue, the thread, context packs and run tokens; every surface (dashboard, Slack, GitHub) is a provider package under `backend/app/integrations/` that implements the ports in `agent/ports.go` and is registered by name from `cmd/run.go`. `go list -deps ./app/agent | grep integrations` must stay empty.

- **Ports** (`agent/ports.go`): `Subject{Kind, Ref, ProjectId}`, `ContextProvider` (evidence per subject kind), `TriggerSource`, `Channel`, `CodeHost`, `IssueTracker`, `Executor`, plus `Identity`, `Link`, `Message`, `InboundMessage`, `Request`, `Field`/`SetupFlow` (what the settings page renders a provider from). `agent.Providers()` aggregates the declarations for `GET /api/integrations/providers`; `agent.SecretFields(p)` names the `secret` fields that `app/secrets` encrypts.
- **StartAttempt** (`agent/attempt.go`) is the only way an attempt begins: dedup on the active attempt for the subject (partial unique index), `number` = max+1 per subject, the project's repository and the chosen or default profile attached, `pending_approval` when asked, origin link + `created` event recorded. Callers wake the queue after commit (`middleware.OnCommit(ctx, agent.Wake)`). `controllers.AttemptLimitHook` (nil = unlimited) runs before it.
- **State machine** (`agent/state.go`): `allowedFrom` per target status; `agent.Transition` is guarded in SQL (`UPDATE ... WHERE status IN (...)`) and records a `status` event; `ErrIllegalTransition` on a lost race or illegal move. Terminal statuses stamp `finished_at` and release the lease.
- **Queue** (`agent/queue.go`): `Claim(executor, limit)` under a 2-minute lease (advisory lock 824737006 on Postgres), `Renew`, `ReclaimStale` (expired leases go back to `queued` with `resume = true` and a `reclaimed` event; `StartReclaimer` runs it every minute), `Wake()` buffered channel.
- **Events** (`agent/events.go`): every payload carries `schemaVersion: 1`; the control plane writes `created`, `status`, `message`, `reclaimed`, `error`; executors append their own kinds with the same envelope.
- **Thread** (`agent/attempt.go`, `agent/dispatch.go`): `agent.Post` stores a message and mirrors it through the outbox to every linked thread of another provider (adapter type `agent:<provider>`, kind `agent`; `agent.OutboxSender` wraps `notifications.AdapterSend` in `cmd/run.go` and calls the registered `Channel.Post`). An inbound message on `needs_input` or `awaiting_review` requeues the attempt with `resume`. The agent's own messages carry provider `agent`; `web` is the dashboard.
- **Inbound** (`agent.HandleInbound`): trigger requests start attempts only for authors mapped to a user (`identities`) with a non-readonly effective role on the project; channel replies are accepted from mapped members on threads the attempt owns (`agent_links`), deduplicated by `external_ref`; code host `pull_request_closed` events flip the linked attempt to `merged` or `closed`.
- **Context pack** (`agent/context.go`): `BuildContextPack` (schemaVersion 1) from validated rows plus the registered `ContextProvider` of the subject kind, previous attempt summaries (report blobs capped at 4000 chars) and the thread; `Render()` writes the prompt with every untrusted string inside `<<<DATA name ... >>>END DATA` blocks and an instruction to treat them as evidence. `agent.TracewayExceptionContext` reads the group, latest occurrences and the distributed trace through `cli/pkg/access` over an in-process loopback client (`mcpmount.LoopbackClient`) authenticated with the attempt's own run token, so the pack never holds more than the agent can read.
- **Run tokens** (`services.GenerateRunToken`, `agent.MintRunToken`): a 15-minute JWT `{kind: run, attemptId, projectId, role: readonly}` signed with `JWT_SECRET`. `AuthenticateBearer` accepts it as a fourth shape and sets `RunAttemptIdContextKey`/`RunProjectIdContextKey` instead of a user; `RequireProjectAccess` honors it for that one project only; `RequireWriteAccess`, admin and org middleware reject it (no user). `POST /api/agent/run-token` always refuses self-renewal. The trusted harness renews through `Local.RenewRunToken` or the runner protocol with a current, unexpired claim and atomically replaces the CLI credential file; a sandbox-held token cannot extend its own lifetime.
- **Secrets**: `Integration.Config` and `AgentProfile.Credential` are `json:"-"` on the models; controllers mask (`secrets.MaskFields`, `********` + `hasSecrets`) and keep stored values on the sentinel (`secrets.KeepStoredFields`). The same helpers back the notification channel masking.
- **Retention**: `AGENT_ATTEMPT_RETENTION_DAYS` (see the env table).
- **Harness** (`backend/app/agentrunner`): `Runner.Run` drives one claimed attempt: `preparing` (mirror fetch + checkout via `agentrunner/workspace`, run token, context pack, the sandboxed CLI logged in by writing a profile into the sandbox's own `XDG_CONFIG_HOME`/`XDG_STATE_HOME`), `running` (the adapter's argv inside `sandbox.Backend.Build` with the workspace writable, sibling workspaces hidden, the skill and CLI read-only, exactly the allowlisted env, egress = provider + this instance; stdout parsed line by line into events batched every 2 s / 50 events, each flush renewing the lease; a separate ticker renews every 30 s and cancels the run when the lease is lost), then by report status: `question` posts it to the thread and moves to `needs_input`; `analysis` posts a finding and ends `analyzed`; `fixed` moves to `verifying` (scratch guard, diff limit 512 KB / 60 files, secret scan, then the repository's test command in the sandbox with the network off; the first test failure goes back to the agent once as a message, the second ends `analyzed`), then `publishing` (`workspace.Commit` pushes `traceway/fix-<ref>-<number>` with the code host credential in the harness environment only, `CodeHost.OpenPullRequest` draft, link recorded, `awaiting_review`). Blobs: `agent/<id>/transcript.jsonl` (appended across rounds and resumes), `diff.patch`, `report.md`. A resumed attempt on the same executor reuses its workspace and `--resume`s the session id kept in the last `result` event; elsewhere it starts fresh with the thread in the pack. Adapters (`agentrunner/agents`): `claude-code` (`claude -p --output-format stream-json`, `ANTHROPIC_API_KEY` from the profile credential, `ANTHROPIC_BASE_URL` optional) and `command` (any executable: prompt on stdin, transcript then `---REPORT---` then the report on stdout; the pipeline tests script it with shell). `StartEmbedded` (AGENT_MODE=embedded) validates git, `claude --version`, the traceway CLI and the sandbox at boot and registers the `embedded` executor for the preflight. `Dockerfile.agent` is the DuckDB image plus Node, the pinned Claude Code CLI, git, bubblewrap, the skill at `/opt/traceway/skill` and the CLI at `/opt/traceway/bin/traceway`.
