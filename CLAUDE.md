...
- **Native `/api/report` rows** go to the same tables (`clientmodels.ClientTrace.RootSpan`, `ClientSpan.ToSpan`): a run is stored as the root span of a trace, `span_id` = the run's id, `trace_id` = the client's distributed id when it sent one, else the run's id. Its spans hang under it, and a native exception sits on the run's span. The report wire format is unchanged: an exception's `traceId` names its owning run, `distributedTraceId` supplies the shared trace identity, and native run/span IDs remain UUIDs.
