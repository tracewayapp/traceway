...
type HealthDeepResponse struct {
	...
	Agent            *agent.HealthStats       `json:"agent,omitempty"`
}
...
func (h healthDeepController) Get(c *gin.Context) {
	...
	if agentStats, err := agent.HealthSnapshot(); err == nil {
		resp.Agent = agentStats
	} else {
		traceway.CaptureException(fmt.Errorf("failed to load agent health: %w", err))
	}
