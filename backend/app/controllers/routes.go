...
func RegisterControllers(router *gin.RouterGroup) {
	...
	router.GET("/organizations/:organizationId/members/:userId/identities", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, MemberIdentityController.List)
	router.PUT("/organizations/:organizationId/members/:userId/identities/:provider", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, MemberIdentityController.Set)
	router.DELETE("/organizations/:organizationId/members/:userId/identities/:provider", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, MemberIdentityController.Clear)
	...
	// own transactions.
	router.GET("/agent/capabilities", middleware.UseAppAuth, AgentAttemptController.Capabilities)
	...
	router.POST("/agent/attempts/list", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.List)
	router.POST("/agent/attempts/report", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.RequireWriteAccess, AgentAttemptController.ReportExternal)
	router.GET("/agent/attempts/by-subject", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.BySubject)
	router.GET("/agent/attempts/badge", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Badge)
	router.GET("/agent/attempts/:id", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Get)
	router.GET("/agent/attempts/:id/events", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Events)
	router.GET("/agent/attempts/:id/messages", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Messages)
	router.POST("/agent/attempts/:id/messages", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.RequireWriteAccess, middleware.Transactional, AgentAttemptController.PostMessage)
	router.POST("/agent/attempts/:id/cancel", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.RequireWriteAccess, middleware.Transactional, AgentAttemptController.Cancel)
	router.POST("/agent/attempts/:id/approve", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.RequireWriteAccess, middleware.Transactional, AgentAttemptController.Approve)
	router.GET("/agent/attempts/:id/diff", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Diff)
	router.GET("/agent/attempts/:id/report", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Report)
	router.GET("/agent/attempts/:id/transcript", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentAttemptController.Transcript)
	// The bearer here is a run token (UseAppAuth accepts it); no project
	// middleware because the attempt id comes from the token itself.
	router.POST("/agent/run-token", middleware.UseAppAuth, middleware.Transactional, AgentAttemptController.RenewRunToken)
	router.GET("/agent-profiles", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, AgentProfileController.ListForProject)
	router.GET("/repositories", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, RepositoryController.Get)
	router.PUT("/repositories", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.RequireWriteAccess, middleware.Transactional, RepositoryController.Upsert)
	router.DELETE("/repositories", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.RequireWriteAccess, middleware.Transactional, RepositoryController.Delete)
	router.GET("/integrations/providers", middleware.UseAppAuth, IntegrationController.Providers)
	router.POST("/integrations/github/manifest", middleware.UseAppAuth, middleware.Transactional, GitHubManifestController.Start)
	router.GET("/integrations/github/manifest/start", middleware.RateLimitPerIP(60, time.Minute), GitHubManifestController.Page)
	router.GET("/integrations/github/manifest/callback", middleware.RateLimitPerIP(60, time.Minute), GitHubManifestController.Callback)
	router.GET("/integrations", middleware.UseAppAuth, middleware.RequireProjectAccess, middleware.Transactional, IntegrationController.ListForProject)
	router.POST("/integrations/:provider/inbound/:integrationId", middleware.RateLimitPerIP(600, time.Minute), IntegrationController.Inbound)
	router.GET("/organizations/:organizationId/integrations", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, IntegrationController.List)
	router.POST("/organizations/:organizationId/integrations", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, IntegrationController.Create)
	router.PUT("/organizations/:organizationId/integrations/:id", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, IntegrationController.Update)
	router.DELETE("/organizations/:organizationId/integrations/:id", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, IntegrationController.Delete)
	router.GET("/organizations/:organizationId/agent-profiles", middleware.UseAppAuth, middleware.RequireOrganizationAccess, middleware.Transactional, AgentProfileController.List)
	router.POST("/organizations/:organizationId/agent-profiles", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, AgentProfileController.Create)
	router.PUT("/organizations/:organizationId/agent-profiles/:id", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, AgentProfileController.Update)
	router.POST("/organizations/:organizationId/agent-profiles/:id/default", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, AgentProfileController.SetDefault)
	router.DELETE("/organizations/:organizationId/agent-profiles/:id", middleware.UseAppAuth, middleware.RequireAdminAccess, middleware.Transactional, AgentProfileController.Delete)

	router.POST("/runners/poll", middleware.UseRunnerAuth, SyntheticRunnerController.Poll)
	RegisterAgentRunnerRoutes(router)
