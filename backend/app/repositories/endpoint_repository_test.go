...
func TestEndpointRepository_FindGroupedByEndpoint(t *testing.T) {
	...
	stats, total, err := EndpointRepository.FindGroupedByEndpoint(ctx, projectId, now.Add(-time.Hour), now.Add(time.Hour), 1, 10, "count", "desc", "", nil)
	if err != nil {
...
func TestEndpointRepository_FindGroupedByEndpoint_Search(t *testing.T) {
	...
	stats, total, err := EndpointRepository.FindGroupedByEndpoint(ctx, projectId, now.Add(-time.Hour), now.Add(time.Hour), 1, 10, "count", "desc", "users", nil)
	if err != nil {
