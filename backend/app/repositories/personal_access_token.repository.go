...
func (r *personalAccessTokenRepository) Create(ex lit.Executor, id, token, prefix string, userId int, name string, expiresAt *time.Time) error {
	var expires any
	if expiresAt != nil {
		expires = formatAuthTime(*expiresAt)
	}
	query, args, err := lit.ParseNamedQuery(
		...
		lit.P{
			...
			"revoked":    false,
			"expires_at": expires,
			"created_at": formatAuthTime(time.Now()),
		},
	)
	if err != nil {
		return err
...
func (r *personalAccessTokenRepository) FindActiveByToken(ex lit.Executor, token string) (*ActivePAT, error) {
	query, args, err := lit.ParseNamedQuery(
		db.Driver,
		...
		lit.P{"hash": hashAuthToken(token)},
	)
	if err != nil {
		return nil, err
	}
	...
	expiry, err := parseNullableAuthTime(expiresAt)
	if err != nil {
		return nil, err
	}
	...
	if pat.LastUsedAt, err = parseNullableAuthTime(lastUsed); err != nil {
		return nil, err
	}
	return &pat, nil
}
...
func (r *personalAccessTokenRepository) ListByUser(ex lit.Executor, userId int) ([]*models.PersonalAccessToken, error) {
	...
	for rows.Next() {
		...
		if pat.ExpiresAt, err = parseNullableAuthTime(expiresAt); err != nil {
			return nil, err
		}
		if pat.CreatedAt, err = parseAuthTime(createdAt); err != nil {
			return nil, err
		}
...
func (r *personalAccessTokenRepository) PruneExpired(ex lit.Executor, now time.Time) (int64, error) {
	query, args, err := lit.ParseNamedQuery(
		...
		lit.P{"revoked": true, "cutoff": formatAuthTime(now)},
	)
	if err != nil {
		return 0, err
	}
...
func (r *personalAccessTokenRepository) TouchLastUsed(ex lit.Executor, id string, usedAt time.Time) error {
	query, args, err := lit.ParseNamedQuery(
		...
		lit.P{"used": formatAuthTime(usedAt), "id": id},
	)
	if err != nil {
		return err
	}
