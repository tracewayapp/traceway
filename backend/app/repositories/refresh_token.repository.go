...
func (r *refreshTokenRepository) FindByToken(ex lit.Executor, token string) (*models.RefreshToken, error) {
	...
	if t.UsedAt, err = parseNullableAuthTime(usedAt); err != nil {
		return nil, err
	}
	if t.ExpiresAt, err = parseAuthTime(expiresAt); err != nil {
		return nil, err
	}
	if t.CreatedAt, err = parseAuthTime(createdAt); err != nil {
		return nil, err
	}
	return &t, nil
}
...
func (r *refreshTokenRepository) MarkUsedIfUnused(ex lit.Executor, token string, usedAt time.Time) (int64, error) {
	query, args, err := lit.ParseNamedQuery(
		...
		lit.P{"used": true, "used_at": formatAuthTime(usedAt), "hash": hashAuthToken(token), "active": false},
	)
	if err != nil {
		return 0, err
	}
...
func (r *refreshTokenRepository) PruneExpired(ex lit.Executor, now time.Time) (int64, error) {
	query, args, err := lit.ParseNamedQuery(
		...
		lit.P{
			"cutoff":      formatAuthTime(now),
			"revoked":     true,
			"used":        true,
			"used_cutoff": formatAuthTime(now.Add(-usedRefreshTokenRetention)),
		},
	)
	if err != nil {
		return 0, err
