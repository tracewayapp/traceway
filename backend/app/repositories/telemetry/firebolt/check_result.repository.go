...
var checkResultColumns = []string{"project_id", "check_id", "run_id", "check_type", "recorded_at", "executed_at", "status", "latency_ms", "status_code", "error_message", "executed_by", "screenshot_key", "tls_days_remaining", "output_key"}
...
func (r *checkResultRepository) Insert(ctx context.Context, result shared.CheckResult) error {
	...
	return insertRows(ctx, "check_results", checkResultColumns, rows)
}
...
// SeriesByCheck buckets one check's results.
func (r *checkResultRepository) SeriesByCheck(ctx context.Context, projectId uuid.UUID, checkId int, from time.Time, to time.Time, intervalMinutes int) ([]*models.CheckSeriesPoint, error) {
	...
	query := fmt.Sprintf(`SELECT CAST(EXTRACT(EPOCH FROM %s) AS BIGINT) AS bucket,
		CAST(COUNT(*) AS BIGINT),
		CAST(SUM(CASE WHEN status = 'up' THEN 1 ELSE 0 END) AS BIGINT),
		CAST(SUM(CASE WHEN status = 'missed' THEN 1 ELSE 0 END) AS BIGINT),
		COALESCE(AVG(CASE WHEN status <> 'missed' THEN latency_ms END), 0),
	...
	GROUP BY bucket ORDER BY bucket ASC`, timeBucketExpr("recorded_at", bucketSeconds))
