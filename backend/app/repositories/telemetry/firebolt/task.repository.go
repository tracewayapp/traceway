...
var taskColumns = []string{"id", "project_id", "task_name", "duration", "recorded_at", "client_ip", "attributes", "app_version", "server_name", "distributed_trace_id", "span_id", "is_root"}
...
func (e *taskRepository) InsertAsync(ctx context.Context, lines []models.Task) error {
	...
	return insertRows(ctx, "tasks", taskColumns, rows)
}
...
func (e *taskRepository) FindGroupedByTaskName(ctx context.Context, projectId uuid.UUID, fromDate, toDate time.Time, page, pageSize int, orderBy string, sortDirection string, search string, rootFilter string) ([]models.TaskStats, int64, error) {
	// is_root is a tasks_dash_idx key and MAX(is_root)/MAX(CASE WHEN
	// is_root = 0 ...) are indexed aggregates, so the whole grouping —
	// including the root filter — merges index states.
	params := minuteRangeParams(projectId, fromDate, toDate)
	whereClause := "project_id = :project_id AND " + indexMinuteRange("recorded_at")
	if search != "" {
		whereClause += " AND STRPOS(LOWER(task_name), LOWER(:search)) > 0"
	...
	groupedCols := `task_name, COUNT(*) as count, AVG(duration) as avg_duration, MAX(recorded_at) as last_seen,
			...
			PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY duration) as p50, PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY duration) as p95`
...
func (e *taskRepository) CountByHour(ctx context.Context, projectId uuid.UUID, start, end time.Time) ([]models.TimeSeriesPoint, error) {
	return queryTaskTimeSeries(ctx,
		`SELECT `+indexBucketExpr("recorded_at", 3600)+` as bucket, CAST(COUNT(*) AS DOUBLE PRECISION) as agg_value
		FROM tasks WHERE project_id = :project_id AND `+indexMinuteRange("recorded_at")+`
...
func (e *taskRepository) AvgDurationByHour(ctx context.Context, projectId uuid.UUID, start, end time.Time) ([]models.TimeSeriesPoint, error) {
	return queryTaskTimeSeries(ctx,
		`SELECT `+indexBucketExpr("recorded_at", 3600)+` as bucket, AVG(duration) / 1000000.0 as agg_value
		FROM tasks WHERE project_id = :project_id AND `+indexMinuteRange("recorded_at")+`
		GROUP BY bucket ORDER BY bucket ASC`,
		minuteRangeParams(projectId, start, end))
}
...
func (e *taskRepository) CountByInterval(ctx context.Context, projectId uuid.UUID, start, end time.Time, intervalMinutes int) ([]models.TimeSeriesPoint, error) {
	return queryTaskTimeSeries(ctx,
		`SELECT `+indexBucketExpr("recorded_at", intervalMinutes*60)+` as bucket, CAST(COUNT(*) AS DOUBLE PRECISION) as agg_value
		FROM tasks WHERE project_id = :project_id AND `+indexMinuteRange("recorded_at")+`
		GROUP BY bucket ORDER BY bucket ASC`,
		minuteRangeParams(projectId, start, end))
}
...
func (e *taskRepository) AvgDurationByInterval(ctx context.Context, projectId uuid.UUID, start, end time.Time, intervalMinutes int) ([]models.TimeSeriesPoint, error) {
	return queryTaskTimeSeries(ctx,
		`SELECT `+indexBucketExpr("recorded_at", intervalMinutes*60)+` as bucket, AVG(duration) / 1000000.0 as agg_value
		FROM tasks WHERE project_id = :project_id AND `+indexMinuteRange("recorded_at")+`
		GROUP BY bucket ORDER BY bucket ASC`,
		minuteRangeParams(projectId, start, end))
}
...
func (e *taskRepository) GetTaskStats(ctx context.Context, projectId uuid.UUID, taskName string, start, end time.Time) (*models.TaskDetailStats, error) {
	...
	statsRow, err := lit.SelectSingleNamed[taskCountStatsRow](db.TelemetryDB,
		"SELECT COUNT(*) AS count, CASE WHEN COUNT(*) > 0 THEN AVG(duration) / 1000000.0 ELSE 0 END AS avg_dur_ms FROM tasks WHERE project_id = :project_id AND task_name = :task_name AND "+indexMinuteRange("recorded_at"),
		params)
	if err != nil {
		return nil, err
	}
	...
	pctQuery, pctArgs, err := parseNamed(
		...
		FROM tasks WHERE project_id = :project_id AND task_name = :task_name AND `+indexMinuteRange("recorded_at"),
		params)
	if err != nil {
		return nil, err
	}
...
// native TIMESTAMP (time.Time) — the Firebolt driver returns TIMESTAMP columns
// directly as time.Time, so no string parsing of the bucket is needed (unlike the SQLite path).
