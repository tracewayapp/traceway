...
func TestOtelSpanIndexedQueries(t *testing.T) {
	...
	t.Logf("SQLite canonical ingest: %d spans in %s", len(spans), time.Since(start))
	if _, err := db.TelemetryDB.Exec("ANALYZE spans_v2"); err != nil {
		t.Fatal(err)
	}
	lookup := shared.SpanLookup{ProjectId: project, TraceId: spans[0].TraceId, SpanId: spans[0].SpanId, RecordedAt: &spans[0].RecordedAt}
	query, args := shared.OtelTopologyQuery([]shared.SpanLookup{lookup}, false, 0, func(t time.Time) any { return sqlitetypes.NewSQLiteTime(t) })
	rows, err := db.TelemetryDB.Query("EXPLAIN QUERY PLAN "+query, args...)
	if err != nil {
		t.Fatal(err)
	}
	var plan string
	...
	if !strings.Contains(plan, "USING INDEX idx_spans_v2_project_id_trace_id_recorded_at") {
		t.Fatalf("unindexed source lookup: %s", plan)
	}
	t.Log("source lookup uses USING INDEX idx_spans_v2_project_id_trace_id_recorded_at")
	...
	t.Logf("SQLite canonical lookup: %s per detail (100 reads, 10000 stored spans)", time.Since(start)/100)
	for _, statement := range []string{"DROP INDEX idx_spans_v2_project_id_trace_id_recorded_at", "DELETE FROM spans_v2"} {
