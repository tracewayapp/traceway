...
func (r *profileRepository) InsertProfilesAsync(ctx context.Context, profiles []models.Profile) error {
	...
	for _, p := range profiles {
		...
		query, args, err := lit.ParseNamedQuery(db.Driver,
			"INSERT INTO profiles (id, project_id, recorded_at, duration, service_name, profile_type, unit, is_gauge, sample_count, total_value, server_name, app_version, attributes, storage_key, trace_id, span_id) VALUES (:id, :project_id, :recorded_at, :duration, :service_name, :profile_type, :unit, :is_gauge, :sample_count, :total_value, :server_name, :app_version, :attributes, :storage_key, :trace_id, :span_id)",
			lit.P{
				"id":           p.Id,
