...
const SCHEMA_CODECS: &str = include_str!("../../../db/firebolt-s3/schema.sql");
...
pub struct FireboltSink {
    ...
    snappy: bool,
    // Operator deployments: one gateway URL, engines picked per request via
    // the X-Firebolt-Engine header. None = single-node, no header.
    write_engine: Option<String>,
    query_engine: Option<String>,
    maint_engine: Option<String>,
    codecs: bool,
}
...
impl FireboltSink {
    pub fn new(url: String, cli: &crate::cli::Cli) -> anyhow::Result<Self> {
        ...
        let codecs = match cli.fb_schema.as_str() {
            "zstd" => false,
            "codecs" => true,
            other => anyhow::bail!("--fb-schema {other} is not a schema variant; expected zstd or codecs"),
        };
        Ok(Self {
            ...
            write_engine: cli.fb_write_engine.clone(),
            query_engine: cli.fb_query_engine.clone(),
            maint_engine: cli.fb_maint_engine.clone(),
            codecs,
        })
    }

    async fn sql(client: &reqwest::Client, url: &str, sql: &str, timeout: Duration, engine: Option<&str>) -> anyhow::Result<Value> {
        let mut req = client.post(format!("{url}/")).timeout(timeout).query(&[("output_format", "JSON_Compact")]).body(sql.to_string());
        if let Some(e) = engine {
            req = req.header("X-Firebolt-Engine", e);
        }
    ...
    async fn table_stats(client: &reqwest::Client, url: &str, engine: Option<&str>) -> anyhow::Result<Value> {
        let v = Self::sql(client, url, "SELECT * FROM information_schema.tables WHERE table_name = 'points'", Duration::from_secs(60), engine).await?;
    ...
    fn eng_write(&self) -> Option<&str> {
        self.write_engine.as_deref()
    }

    fn eng_maint(&self) -> Option<&str> {
        self.maint_engine.as_deref().or(self.write_engine.as_deref())
    }
}
...
async fn upload(client: &reqwest::Client, url: &str, sql: &str, part: &str, bytes: Vec<u8>, engine: Option<&str>) -> Result<(), WriteError> {
    ...
    let mut req = client.post(format!("{url}/")).query(&[("output_format", "JSON_Compact")]).multipart(form);
    if let Some(e) = engine {
        req = req.header("X-Firebolt-Engine", e);
    }
    let resp = req.send().await?;
...
impl Sink for FireboltSink {
    ...
    async fn setup(&self, reset: bool, timeout: Duration) -> anyhow::Result<()> {
        ...
        let eng = self.write_engine.clone();
        wait_until(timeout, || {
            let c = c.clone();
            let url = url.clone();
            let eng = eng.clone();
            async move { Self::sql(&c, &url, "SELECT 1", Duration::from_secs(10), eng.as_deref()).await.map(|v| v.get("data").is_some()).unwrap_or(false) }
        })
        .await?;
        if reset {
            Self::sql(&self.client, &self.url, "DROP TABLE IF EXISTS points", Duration::from_secs(300), self.eng_write()).await?;
            Self::sql(&self.client, &self.url, "DROP TABLE IF EXISTS series", Duration::from_secs(300), self.eng_write()).await?;
        }
        let schema_src = if self.codecs { SCHEMA_CODECS } else { SCHEMA };
        for stmt in super::split_statements(schema_src) {
            Self::sql(&self.client, &self.url, &stmt, Duration::from_secs(60), self.eng_write()).await?;
        }
        Ok(())
    }
    ...
    async fn register_series(&self, cat: &SeriesCatalog) -> anyhow::Result<()> {
        ...
        while id < n {
            ...
            upload(&self.client, &self.url, "INSERT INTO series SELECT series_id, name, tags FROM READ_PARQUET('upload://series')", "series", bytes, self.eng_write())
                .await
    ...
    async fn reader(&self) -> anyhow::Result<Box<dyn SinkReader>> {
        Ok(Box::new(FbReader { client: client(Duration::from_secs(120)), url: self.url.clone(), engine: self.query_engine.clone() }))
    }

    async fn health(&self) -> anyhow::Result<Value> {
        let mut v = Self::table_stats(&self.client, &self.url, self.eng_write()).await.unwrap_or_else(|e| json!({ "error": e.to_string() }));
    ...
    async fn is_reachable(&self) -> bool {
        Self::sql(&self.client, &self.url, "SELECT 1", Duration::from_secs(10), self.eng_write()).await.is_ok()
    }

    async fn settle(&self, deadline: Instant) -> SettleReport {
        let mut rep = SettleReport::default();
        let mut prev = Self::table_stats(&self.client, &self.url, self.eng_maint()).await.ok().and_then(|s| Self::tablets(&s));
        for pass in 0..6 {
            let t0 = Instant::now();
            let r = Self::sql(&self.client, &self.url, "VACUUM points", deadline.saturating_duration_since(Instant::now()).max(Duration::from_secs(10)), self.eng_maint()).await;
            let now = Self::table_stats(&self.client, &self.url, self.eng_maint()).await.ok().and_then(|s| Self::tablets(&s));
    ...
    async fn db_size(&self) -> anyhow::Result<DbSize> {
        let s = Self::table_stats(&self.client, &self.url, self.eng_write()).await?;
    ...
    fn settings(&self) -> Value {
        json!({
            "stage": "upload:// multipart parquet",
            "parquet_snappy": self.snappy,
            "vacuum_every_inserts": self.vacuum_every,
            "max_tablets": self.max_tablets,
            "schema": if self.codecs { "codecs" } else { "zstd" },
            "write_engine": self.write_engine,
            "query_engine": self.query_engine,
            "maint_engine": self.maint_engine,
        })
    }
}
...
impl SinkWriter for FbWriter {
    async fn write(&mut self, batch: &Batch, _cat: &SeriesCatalog) -> Result<Ack, WriteError> {
        ...
        upload(&self.sink.client, &self.sink.url, "INSERT INTO points SELECT series_id, ts, value FROM READ_PARQUET('upload://batch')", "batch", bytes, self.sink.eng_write()).await?;
        ...
        if self.maintainer {
            ...
            let over = if !due && n % 5 == 0 {
                FireboltSink::table_stats(&self.sink.client, &self.sink.url, self.sink.eng_maint()).await.ok().and_then(|s| FireboltSink::tablets(&s)).map(|t| t > self.sink.max_tablets).unwrap_or(false)
            } else {
                false
            };
            if due || over {
                let t0 = Instant::now();
                if let Err(e) = FireboltSink::sql(&self.sink.client, &self.sink.url, "VACUUM points", Duration::from_secs(1800), self.sink.eng_maint()).await {
...
impl SinkReader for FbReader {
    async fn visible_max_ts(&mut self) -> anyhow::Result<Option<i64>> {
        let v = FireboltSink::sql(&self.client, &self.url, &format!("SELECT CAST(max(ts) AS TEXT) FROM points WHERE series_id = {SENTINEL_ID}"), Duration::from_secs(60), self.engine.as_deref()).await?;
    ...
    async fn run_query(&mut self, q: &QueryInstance, deadline: Duration) -> QueryOutcome {
        ...
        let t0 = Instant::now();
        match FireboltSink::sql(&self.client, &self.url, &sql, deadline + Duration::from_secs(2), self.engine.as_deref()).await {
            Ok(v) => {
