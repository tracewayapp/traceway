...
def render_html(runs, heads, tier, svgs, slow_ms):
    ...
    H.append("<p>What each store lets pile up while still acknowledging writes: merges for ClickHouse and VictoriaMetrics, the write-ahead log above its checkpoint threshold for DuckDB (below it the WAL is a sawtooth by design), tablets awaiting vacuum for Firebolt. Flat is healthy; a rising line during the fill is the store falling behind no matter what the acknowledged rate says.</p>")
    H.append('<div class="grid">')
    for h in valid:
    ...
    H.append("<li><b>Deferred work / debt</b>: active parts (ClickHouse), small plus in-memory parts (VictoriaMetrics), WAL bytes above the checkpoint threshold (DuckDB), tablets (Firebolt). During the fill, a trailing 10 minute mean more than 30% above the previous 10 minutes for a full minute, or any throttled write, counts as fallen behind.</li>")
