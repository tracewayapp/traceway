...
1. A trace is created — either automatically by middleware (endpoint) or manually via `StartTrace` / `MeasureTask` (task)
...
Error traces (panics, status >= 500, Gin errors) use the error sample rate, which you typically keep at 100% to capture all problems.
