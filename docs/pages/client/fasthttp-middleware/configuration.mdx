...
Long-lived streaming routes have a request lifetime that equals the connection duration, not request processing latency. The middleware **auto-detects** streaming responses from the response headers (SSE `Content-Type: text/event-stream`, WebSocket upgrade status `101` or `Upgrade: websocket`) and excludes them from P50/P95/P99, Apdex, and impact scoring while keeping their status code, error rate, and throughput visible on the dashboard.

For paths the SDK can't auto-detect:

```go
tracewayfasthttp.New(
    connectionString,
    tracewayfasthttp.WithStreamingPaths("/api/long-poll", "/api/stream/json"),
)
```
...
```go
func eventsHandler(ctx *fasthttp.RequestCtx) {
    tracewayfasthttp.MarkStream(ctx)
    // ... write stream ...
}
```
