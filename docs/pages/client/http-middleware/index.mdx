...
<Callout type="warning">
  The Traceway Go SDK is no longer the recommended way to integrate with Traceway. For new projects, instrument with [OpenTelemetry](/client/otel) — it works across languages and Traceway natively ingests OTLP. The Go SDK is still supported, but new features land in the OTel path first.
</Callout>
...
- [Configuration](/client/http-middleware/configuration) — All options and passthrough settings
- [Error Recording](/client/http-middleware/error-recording) — Control what request data is captured on errors
