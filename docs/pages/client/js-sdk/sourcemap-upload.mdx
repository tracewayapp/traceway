...
There is nothing to configure at runtime. When an exception is captured, Traceway takes each minified frame's filename and looks up the map you uploaded under that same name, deobfuscating the frame. The most recent upload of each filename wins, so content-hashed bundle names (the default in Vite, Next, Angular, and most bundlers) keep every build's maps distinct automatically.
