...
The `@traceway/backend` package is a complete Node.js SDK for server-side applications. It provides error tracking, distributed tracing, span management, and metrics collection.

## Installation

```bash
npm install @traceway/backend
```
...
For browser applications, use [`@traceway/frontend`](/client/js-sdk) or framework-specific packages.
...
```typescript
// Core functions
export { init, shutdown } from "@traceway/backend";

// Exception capture
export { captureException, captureExceptionWithAttributes, captureMessage } from "@traceway/backend";
...
// Trace capture
export { captureCurrentTrace, captureTrace, captureTask } from "@traceway/backend";

// Span management
export { startSpan, endSpan, addSpanToContext, getTraceSpans, getTraceDuration } from "@traceway/backend";

// Metrics
export { captureMetric } from "@traceway/backend";

// Task measurement
export { measureTask } from "@traceway/backend";

// Types
export type { TracewayOptions, TraceContext, TraceContextOptions, SpanHandle } from "@traceway/backend";
```
