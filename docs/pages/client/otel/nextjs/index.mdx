...
**Backend.** Install the OTel Node SDK and add a single `instrumentation.ts`. This installs all three signals at once: traces, metrics, and logs.
...
The `process.env.NEXT_RUNTIME !== "nodejs"` guard is required. Next.js runs `instrumentation.ts` in both the Node.js and Edge runtimes, and the OTel Node SDK only works in Node.js. The dynamic `import()` calls keep OTel out of the Edge bundle.
...
Logs are a separate OTLP pipeline. The Quick Start `instrumentation.ts` already configures it. Create one shared module for the logger:
...
const nextConfig: NextConfig = {
  serverExternalPackages: ["ioredis", "redis", "mysql2"],
};
...
Use `instrumentation.ts` with the `register()` hook, as shown in the Quick Start.
...
Next.js installs its own `Error.prepareStackTrace`, which bypasses Node's source map support, so the stack recorded by OTel keeps the bundled frames. Reset it at the top of `register()` to get Node's default formatter back:
...
With these set, your `instrumentation.ts` can omit the exporter URLs, the headers, and the `resource` block. The SDK picks all of them up automatically.
