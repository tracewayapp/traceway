...
Each Endpoint / Task / AI Trace row carries an `is_root` flag derived from whether its span has a parent. The detail page groups the entities sharing the span's actual `traceId`, including cross-service requests. Standard W3C context propagation connects browser, server and downstream spans. OTel span links are retained on spans and can reference other traces; they do not merge those traces.
