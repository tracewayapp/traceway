...
Integrate Traceway into your React Native or Expo application with the [`@tracewayapp/react-native`](https://www.npmjs.com/package/@tracewayapp/react-native) package. Plain JavaScript (no native modules, no `pod install`, no Gradle changes) that works in Expo Go, in bare React Native CLI projects, and in EAS builds.
...
| `beforeCapture` | `(exception) => boolean` | none | Return `false` to suppress an exception |
