...
The `traceway_gin` package provides middleware for the [Gin](https://gin-gonic.com/) web framework. It automatically creates a trace for each request, captures panics, and detects errors from Gin's error chain.
...
- [Configuration](/gin-middleware/configuration) — All options including route filtering and passthrough settings
- [Error Recording](/gin-middleware/error-recording) — Control what request data is captured on errors
