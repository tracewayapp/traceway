...
4. **Linked across the trace.** An AI trace keeps the OTel trace id of its span, so a child AI trace and its parent endpoint show up together in the distributed-trace view with no extra configuration.
...
3. The trace id. A single agent run inside one request still groups its calls, even without an explicit id.
