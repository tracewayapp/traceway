...
<Callout type="info">
Traceway also ships a [fix agent](/learn/agent) that runs the same loop on the server: a sandbox, a conversation on the issue page and in Slack, and a draft pull request without any CI setup. The CI contract on this page keeps working next to it and will report its runs into the same attempts.
</Callout>
...
## Attempts on the issue page

The publish step also records the run as a fix-agent [attempt](/learn/agent/fix-it) when `traceway_token` and `project_id` are set (input `report_attempt`, on by default): it calls `traceway agent report` with the outcome, the branch and the pull request, so a CI-only setup shows its runs on the issue page and on the Agent page next to the server-side agent's, with the report as the attempt's thread entry. The attempt's executor is `ci`.
