...
| `oltp_pg telemetry_ch` | **ClickHouse + PostgreSQL** — standalone server mode. |
...
```bash
cd backend
go build -tags "oltp_pg telemetry_ch" ./...
go run -tags "oltp_pg telemetry_ch" ./cmd/traceway
```
...
This builds a Docker image with both databases, applies all migrations, and runs the test suite with `-tags "oltp_pg telemetry_ch"`. All tests should pass.
...
**Repositories** are singletons (`var FooRepository = &fooRepository{}`). Telemetry repositories have one implementation per backend under `backend/app/repositories/telemetry/{clickhouse,sqlite,duckdb}/`, selected by the `telemetry_*` build tags and re-exported through the `repositories` package so callers never import a backend package directly.
...
go build -tags "oltp_pg telemetry_ch" ./...       # ClickHouse + PostgreSQL
