...
- When a trace crosses service boundaries, the **Related Traces** tab also resolves linked trace IDs within the selected project and surrounding time window.
...
For API callers, `POST /api/logs` accepts `traceId` for an exact trace and retains `distributedTraceId` for bounded linked-trace expansion in the selected project. `excludeTraceId` removes one trace from that expansion and requires `distributedTraceId`. Invalid IDs return an error rather than broadening the query.
