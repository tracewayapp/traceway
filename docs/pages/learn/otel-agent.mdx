...
The agent also attaches resource attributes to the OTLP payload: `service.name`, `host.name`, `os.type`, and on cloud VMs `cloud.provider`, `cloud.region`, and `cloud.account.id` (EC2 / GCE / Azure are auto-detected). Traceway lifts `service.name` onto metric tags, renamed to `server_name`. Every other resource attribute is dropped unless it is on a fixed allowlist. For this agent the allowlist matters only when `TRACEWAY_PROCESS_NAMES` is set: `process.pid`, `process.executable.name`, `process.command_line` and `process.owner` are lifted, which is what keeps the per-process series apart. `host.name`, `os.type` and the `cloud.*` attributes are never stored as tags. See [Resource Attributes](/client/otel/metrics#resource-attributes) for the full allowlist.
