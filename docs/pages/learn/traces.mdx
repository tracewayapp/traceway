...
- A request and its queue worker share a trace id when the instrumentation propagates a parent context. Some queue integrations start a new trace with a link instead; those runs are not grouped by shared trace ID. They show up as two rows (the endpoint and the task), and the Distributed Trace card on either page shows both, the task nested under the endpoint.
