...
Every row keeps the ids of the span it was made from: `trace_id`, `span_id` and `parent_span_id`. Its own row id is derived from the project, the trace and the span, so a retried export produces the same id.
...
- A request that dispatches a queue job and the worker that processes it share a trace id. They show up as two rows (the endpoint and the task), and the Distributed Trace card on either page shows both, the task nested under the endpoint.
...
You don't need to set any vendor attribute for this to work, OTLP's native trace id is enough. One case needs a link between two different trace ids: a browser request and the backend trace it started. There the span attribute `traceway.distributed_trace_id` names the other trace. It is stored as the row's `linked_trace_id`, and opening either trace shows both. See [Distributed Tracing](/client/js-sdk/distributed-tracing).
