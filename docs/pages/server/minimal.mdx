...
| `TRUSTED_PROXIES` | Comma-separated IPs/CIDRs of reverse proxies whose `X-Forwarded-For` is trusted when resolving the client IP for the per-IP rate limits. Defaults to loopback (`127.0.0.1,::1`); add your proxy's address or network when it connects from elsewhere, or `*` to trust any peer (lets clients spoof their IP past the limits). | `172.16.0.0/12` |
| `TRUSTED_PROXY_HEADER` | A header trusted verbatim as the client IP, taking precedence over `TRUSTED_PROXIES` — e.g. behind ingress-nginx or Cloudflare. Only safe when every request provably passes through a proxy that overwrites the header. | `X-Real-IP` |
