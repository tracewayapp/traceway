...
| `SECRETS_KEY` | *(key file on SQLite; required on PostgreSQL)* | Base64 of 32 random bytes (`openssl rand -base64 32`) that encrypts stored credentials at rest: notification channel tokens today, integration and repository credentials for the fix agent. When unset on a SQLite deployment, the server generates `traceway.secrets.key` (mode 600) next to the main database on first boot and logs a warning; move that value into `SECRETS_KEY` and back it up, since credentials cannot be decrypted without it. On PostgreSQL the variable is required and startup exits 1 without it. |
