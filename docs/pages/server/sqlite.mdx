...
| `TRUSTED_PROXIES` | `127.0.0.1,::1` | Comma-separated IPs/CIDRs of reverse proxies whose `X-Forwarded-For` is trusted when resolving the client IP for the per-IP rate limits (login, registration, password reset, device auth, public status pages). The loopback default covers a proxy on the same host; add your proxy's address or network (e.g. `172.16.0.0/12` for Docker bridge networks) when it connects from elsewhere, or `*` to trust any peer — which lets clients spoof their IP and rotate out of the limits |
| `TRUSTED_PROXY_HEADER` | *(none)* | A header trusted verbatim as the client IP, taking precedence over `TRUSTED_PROXIES` — e.g. `X-Real-IP` behind ingress-nginx or `CF-Connecting-IP` behind Cloudflare. Only safe when every request provably passes through a proxy that overwrites the header |
