...
| Compiled `.tw` resolvers | same keys with the `.map`/`.js` suffix replaced by `.tw` | the [symbolication cache](/learn/symbolication-js#cache-backends-memory-and-disk), automatically |
...
- [Symbolication (JS)](/learn/symbolication-js): how `.tw` resolvers and the cache tiers use this storage
