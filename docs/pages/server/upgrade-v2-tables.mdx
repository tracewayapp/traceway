...
5. Replace or stop every old backend writer before enabling backfill. With multiple replicas, enable the worker on exactly one upgraded instance. The progress table does not provide a distributed lock.
...
Stop the backfill before switching binaries or images. This migration preserves legacy tables, so a previous backend can read the legacy history still present. It cannot read telemetry written only to V2, and there is no reverse backfill. Those V2 rows remain available to the new release while retained.
...
- Endpoints, tasks and AI traces carry `traceId`, `spanId`, `parentSpanId` and `linkedTraceId` as hex strings. Their `distributedTraceId` field is gone.
...
- `POST /api/logs` retains `distributedTraceId` and `excludeTraceId` for linked-trace expansion within the selected project. `traceId` searches one exact trace.
- The span attribute `traceway.distributed_trace_id` is stored as a link to another trace instead of replacing the source trace ID.
