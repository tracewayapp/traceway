...
This is dispatched by the trace, not configured per project: a browser exception resolves against source maps, a Flutter exception against its symbols, and the engine picks the path from the SDK language and the shape of the trace. The collector processor and the library do the same work at a different point in the pipeline.
