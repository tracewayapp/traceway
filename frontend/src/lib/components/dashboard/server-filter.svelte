<script lang="ts">
	...
	const filteredServers = $derived(
		[...availableServers].sort().filter((s) => s.toLowerCase().includes(searchQuery.toLowerCase()))
	);
...
<Popover.Root bind:open={isOpen} onOpenChange={handleOpenChange}>
		...
		<Button variant="outline" size="sm" class="h-auto min-h-8 border-dashed py-1.5">
			...
			{#if displayServers().length > 0}
				...
				<div class="flex flex-wrap gap-1">
					{#each displayServers() as server, __index (__index)}
						{@const color = serverColorMap[server] || '#888888'}
						<Badge
							variant="secondary"
							class="flex items-center gap-1 rounded-sm px-1.5 py-0 font-normal"
						>
							<span class="h-2 w-2 flex-shrink-0 rounded-full" style="background-color: {color};"
							></span>
							<span class="max-w-[80px] truncate">{server}</span>
						</Badge>
					{/each}
				</div>
	...
	<Popover.Content class="w-[200px] p-0" align="start">
		<!-- Search Input -->
		<div class="p-2">
			<Input placeholder="Servers" bind:value={searchQuery} class="h-8" />
		</div>

		<!-- Quick Actions -->
		{#if availableServers.length > 0}
			<div class="flex items-center justify-between border-t px-2 py-1.5">
				<button class="text-xs text-muted-foreground hover:text-foreground" onclick={selectAll}>
					Select all
				</button>
				<button class="text-xs text-muted-foreground hover:text-foreground" onclick={selectNone}>
					Deselect all
				</button>
			</div>
		{/if}
		...
		<div class="max-h-[200px] overflow-y-auto">
			{#each filteredServers as serverName, __index (__index)}
				...
				>
					<div
						class="flex h-4 w-4 items-center justify-center rounded-sm border border-primary {selected
			...
			{#if filteredServers.length === 0}
				<div class="py-6 text-center text-sm text-muted-foreground">No servers found.</div>
			{/if}
		</div>
		...
		{#if hasActiveFilters}
			<Separator />
			<button class="w-full py-2 text-center text-sm hover:bg-accent" onclick={clearFilters}>
				Clear filters
			</button>
		{/if}
	</Popover.Content>
