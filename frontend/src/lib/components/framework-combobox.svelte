<script lang="ts">
	...
	const frameworks = [
		...
		{ value: 'drift', label: 'Drift', description: 'Drift', group: 'Go' },
	...
	const jsFrameworks = $derived(
		filteredFrameworks.filter((f) => {
			// @ts-ignore
			return f.group === 'JavaScript';
		})
	);
			...
			<Combobox.Viewport class="max-h-72 w-full scroll-my-1 overflow-y-auto p-1">
				...
				{:else}
					...
					{#if jsFrameworks.length > 0}
						<Combobox.Group>
							<Combobox.GroupHeading class="px-2 py-1.5 text-xs font-semibold text-muted-foreground"
								>JavaScript</Combobox.GroupHeading
							>
