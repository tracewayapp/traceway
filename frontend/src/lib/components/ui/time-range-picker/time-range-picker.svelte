<script lang="ts">
		...
		if (fromDateChanged || toDateChanged || fromTimeChanged || toTimeChanged) {
			...
			if (!isOpen && !isApplyingInternally) {
				...
				tempFromDateTime = new CalendarDateTime(
					...
					fromDate.month,
					fromDate.day,
					Number.isFinite(fromHour) ? fromHour : 0,
					Number.isFinite(fromMinute) ? fromMinute : 0,
					0
				);
				tempToDateTime = new CalendarDateTime(
					toDate.year,
					toDate.month,
					toDate.day,
					Number.isFinite(toHour) ? toHour : 23,
					Number.isFinite(toMinute) ? toMinute : 59,
					0
				);
			}
		}
	...
	let tempFromDateTime = $state<CalendarDateTime>(
		new CalendarDateTime(
			...
			fromDate.month,
			fromDate.day,
			initialFromTimeParts.hour,
			initialFromTimeParts.minute,
			0
		)
	);
	let tempToDateTime = $state<CalendarDateTime>(
		new CalendarDateTime(
			...
			toDate.month,
			toDate.day,
			initialToTimeParts.hour,
			initialToTimeParts.minute,
			59
		)
	);
	...
	function applyAndClose() {
		...
		fromDate = new CalendarDate(
			tempFromDateTime.year,
			tempFromDateTime.month,
			tempFromDateTime.day
		);
	...
	function handleOpenChange(open: boolean) {
		if (open) {
			...
			initialFromDateTime = new CalendarDateTime(
				...
				tempFromDateTime.month,
				tempFromDateTime.day,
				tempFromDateTime.hour,
				tempFromDateTime.minute,
				tempFromDateTime.second
			);
			initialToDateTime = new CalendarDateTime(
				tempToDateTime.year,
				tempToDateTime.month,
				tempToDateTime.day,
				tempToDateTime.hour,
				tempToDateTime.minute,
				tempToDateTime.second
			);
		} else {
...
<Popover.Root bind:open={isOpen} onOpenChange={handleOpenChange}>
	<Popover.Trigger class="w-full">
		<Button variant="outline" class="h-9 w-full justify-between gap-2 font-normal">
	...
	<Popover.Content class="w-auto p-0" align="start">
		<div class="flex">
			...
			<div class="w-[140px] border-r py-2">
				{#each presetGroups as group, groupIndex (groupIndex)}
					{#if groupIndex > 0}
						<div class="mx-2 my-1 border-t"></div>
					{/if}
					{#each group.presets as preset, __index (__index)}
						<button
							class="flex w-full items-center justify-between px-3 py-1.5 text-left text-sm transition-colors hover:bg-muted/50 {selectedPreset ===
				...
				<div class="mx-2 my-1 border-t"></div>
				<button
					class="flex w-full items-center justify-between px-3 py-1.5 text-left text-sm transition-colors hover:bg-muted/50 {showCustom
						? 'bg-muted'
						: ''}"
			...
			<!-- Custom Section -->
			{#if showCustom}
				<div class="min-w-[380px] p-5">
					<div class="mb-3 flex items-center justify-between">
						<span class="text-xs font-medium tracking-wide text-muted-foreground uppercase"
							>Custom</span
						>
						<button class="text-xs text-primary hover:underline" onclick={resetToNow}>
							Reset to now
						</button>
					</div>
					...
					<div class="mb-4 flex items-center gap-2">
						...
						<Popover.Root bind:open={fromPickerOpen}>
							<Popover.Trigger>
								<button
									class="flex h-9 items-center gap-2 rounded-md border px-3 text-left text-sm transition-colors hover:bg-muted/50 {fromPickerOpen
										? 'ring-2 ring-ring'
										: ''}"
								>
									<span class="tabular-nums">{formatDateCompact(tempFromDateTime)}</span>
									<span class="text-muted-foreground tabular-nums"
										>{formatTime12h(tempFromDateTime)}</span
									>
								</button>
							</Popover.Trigger>
							<Popover.Content class="w-auto p-0" align="start">
						...
						<Popover.Root bind:open={toPickerOpen}>
							<Popover.Trigger>
								<button
									class="flex h-9 items-center gap-2 rounded-md border px-3 text-left text-sm transition-colors hover:bg-muted/50 {toPickerOpen
										? 'ring-2 ring-ring'
										: ''}"
								>
									<span class="tabular-nums">{formatDateCompact(tempToDateTime)}</span>
									<span class="text-muted-foreground tabular-nums"
										>{formatTime12h(tempToDateTime)}</span
									>
								</button>
							</Popover.Trigger>
							<Popover.Content class="w-auto p-0" align="start">
