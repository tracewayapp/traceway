			...
			{:else if success}
				<div class="flex flex-col items-center justify-center gap-4 py-8">
					...
					<p class="text-center text-lg font-medium">
						Welcome to {invitationInfo?.organizationName}!
					</p>
			...
			{:else if invitationInfo}
				<div class="space-y-4">
					<div class="rounded-md bg-muted/50 p-4 text-sm">
						<p>
							<strong>{invitationInfo.inviterName}</strong> invited you to join as a
							<strong>{getRoleLabel(invitationInfo.role)}</strong>
						</p>
					</div>
					...
					{#if invitationInfo.existsAsUser}
						{#if authState.isAuthenticated}
							<p class="text-center text-muted-foreground">Click below to join the organization</p>
						...
						{:else}
							...
							<Button class="w-full" onclick={handleLoginRedirect}>Login to Accept</Button>
						{/if}
					{:else}
						<form
							onsubmit={(e) => {
