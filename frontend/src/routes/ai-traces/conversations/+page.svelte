<script lang="ts">
    ...
    type SortField = 'turns' | 'tool_call_count' | 'user_id' | 'total_tokens' | 'total_cost' | 'first_seen' | 'last_seen';
    ...
    function parseConversationsUrlParams() {
        if (!browser) return { preset: '24h', from: null, to: null, search: '', flagged: 'all', userId: '', model: '', tool: '' };
    ...
    async function loadData(pushToHistory = true) {
        ...
        try {
            ...
            const response = await api.post('/ai-conversations/grouped', requestBody, { projectId: projectsState.currentProjectId ?? undefined });
    ...
    const filterPills = $derived([
        ...(userIdFilter ? [{ kind: 'user' as FilterPillKind, label: 'user', value: userIdFilter }] : []),
        ...(modelFilter ? [{ kind: 'model' as FilterPillKind, label: 'model', value: modelFilter }] : []),
        ...(toolFilter ? [{ kind: 'tool' as FilterPillKind, label: 'tool', value: toolFilter }] : []),
        ...(flaggedFilter === 'flagged' ? [{ kind: 'flagged' as FilterPillKind, label: 'flagged', value: 'only' }] : [])
    ]);
    ...
    const hasActiveFilters = $derived(
        !!(searchQuery.trim() || userIdFilter || modelFilter || toolFilter || flaggedFilter === 'flagged')
    );
    ...
    const modelFilterLabel = $derived(modelFilter ? shortModelName(modelFilter) : 'All models');
    const toolFilterLabel = $derived(toolFilter || 'All tools');
...
<div class="space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        {#if projectsState.canWriteCurrentProject}
            <Button
                variant="outline"
                size="sm"
                onclick={() => (showFlaggedTermsSettings = true)}
            >
    ...
    >
        {#snippet children()}
            ...
            <Select.Root type="single" bind:value={modelFilter} onValueChange={applyFilterChange}>
                <Select.Trigger class="h-9 w-[160px] rounded-none border-r-0 shadow-none">
                    <span class="truncate">{modelFilterLabel}</span>
                </Select.Trigger>
                <Select.Content>
                    <Select.Item value="" label="All models">All models</Select.Item>
                    {#each facetModels as model}
                        <Select.Item value={model} label={model}>{model}</Select.Item>
                    {/each}
                </Select.Content>
            </Select.Root>
            <Select.Root type="single" bind:value={toolFilter} onValueChange={applyFilterChange}>
                <Select.Trigger class="h-9 w-[150px] rounded-none border-r-0 shadow-none">
                    <span class="truncate">{toolFilterLabel}</span>
                </Select.Trigger>
                <Select.Content>
                    <Select.Item value="" label="All tools">All tools</Select.Item>
                    {#each facetTools as tool}
    ...
    {#if filterPills.length > 0}
        <div class="flex flex-wrap items-center gap-2">
            {#each filterPills as pill (pill.kind)}
                <span class="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 font-mono text-xs">
    ...
    <div class="rounded-md border overflow-hidden">
        <Table.Root>
            ...
            {:else if error}
            <Table.Body>
                <TableEmptyState colspan={8} message={error} />
            </Table.Body>
            ...
            <Table.Header>
                <Table.Row>
                    ...
                    <TracewayTableHeader
                        label="Models"
                        tooltip="Models used in the conversation"
                    />
                    <TracewayTableHeader
                        label="Tokens"
            ...
            <Table.Body>
                {#each conversations as conv}
                    <Table.Row
                        ...
                        onclick={createRowClickHandler(resolve('/ai-traces/conversations/[conversationId]', { conversationId: encodeURIComponent(conv.conversationId) }), 'preset', 'from', 'to')}
                    >
                        ...
                        <Table.Cell class="tabular-nums">
                            {#if isTurnsOutlier(conv)}
                                <Tooltip.Provider>
                                    <Tooltip.Root>
                                        <Tooltip.Trigger>
                                            <span class="inline-flex items-center gap-1 font-semibold text-orange-600 dark:text-orange-400">
                                                {formatCount(conv.turns)}
                                                <TrendingUp class="h-3 w-3" />
                                            </span>
                                        </Tooltip.Trigger>
                                        <Tooltip.Content>
                                            <p class="text-xs">Above the 95th percentile turns for this period (P95: {Math.round(thresholds?.p95Turns ?? 0)})</p>
                                        </Tooltip.Content>
                                    </Tooltip.Root>
                                </Tooltip.Provider>
                            {:else}
                                    ...
                                    <Tooltip.Root>
                                        <Tooltip.Trigger>
                                            <span class="inline-flex items-center gap-1">
                                                <Badge variant="outline" class="font-mono text-xs">{shortModelName(conv.models[0])}</Badge>
                                                {#if conv.models.length > 1}
                                                    <Badge variant="outline" class="text-xs">+{conv.models.length - 1}</Badge>
                                                {/if}
                                            </span>
                                        </Tooltip.Trigger>
                                        <Tooltip.Content>
                            ...
                            {#if isCostOutlier(conv)}
                                <Tooltip.Provider>
                                    <Tooltip.Root>
                                        ...
                                        <Tooltip.Content>
                                            <p class="text-xs">Above the 95th percentile cost for this period (P95: {formatCost(thresholds?.p95Cost ?? 0)})</p>
                                        </Tooltip.Content>
                                    </Tooltip.Root>
                                </Tooltip.Provider>
                            {:else}
