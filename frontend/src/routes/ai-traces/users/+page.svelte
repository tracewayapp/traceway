<script lang="ts">
    ...
    type SortField = 'conversation_count' | 'total_calls' | 'median_turns' | 'avg_conversation_cost' | 'total_cost' | 'total_tokens' | 'flagged_conversation_count' | 'last_seen';
    ...
    async function loadData(pushToHistory = true) {
        ...
        try {
            ...
            const response = await api.post('/ai-users/grouped', requestBody, { projectId: projectsState.currentProjectId ?? undefined });
            ...
            <Table.Body>
                {#each users as user}
                    <Table.Row
                        ...
                        onclick={createRowClickHandler(userConversationsHref(user.userId), 'preset', 'from', 'to')}
                    >
