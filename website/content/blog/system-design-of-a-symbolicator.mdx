...
On the preact pair, `BuildTW` turns a 69 KB map and a 10 KB bundle into a **48 KB** `.tw` blob with about 2,025 tokens. That blob is one allocation the GC never looks inside. It's the unit we cache. The cache is a plain LRU keyed by file name, bounded by entry count *and* total bytes. (LRU means least-recently-used: when it fills up, whatever nobody has touched in the longest gets dropped.)
...
Three stages, one idea. Build the expensive structure once, and give it a shape that's identical in memory and on disk. After that, caching it, persisting it, sharing it across machines, and searching it without a single copy all collapse into the same flat bytes handed to the same function. The payoff is concrete: `oxc` plus `disk` runs about 32× the industry-standard library under real churn, at a twelfth of the memory, and stays up on an 8 GB box where the in-memory version falls over. Parse once, look up forever. The 44-nanosecond lookup is what you came for, and the whole art is making sure it's almost the only thing you ever run.

A symbolicator is the least glamorous corner of an observability platform. Nobody adopts a tool because its stack-trace decoder is elegant, and most products treat this layer as plumbing to wave at and move past. I sank weeks into it anyway, reading source maps and DWARF byte by byte before I trusted myself to write the fast version, because "close enough" symbolication is exactly what fails you at 3 a.m., staring at a frame that says `n` when you need it to say `validateCheckout`. Getting the boring layer right, in every language, is the whole reason I'm building Traceway from the ground up instead of renting it. That's the part I find genuinely fun.

And the piece that made the whole thing click was `mmap`. I'd known about it for years and never once reached for it in production until this. Here it was load-bearing: you hand the kernel a file, it hands back a byte array that *is* the file, and then it quietly handles eviction for you, logic I never had to write and so can't get wrong. The fallback, `os.ReadFile` on platforms without it, is perfectly correct, but it copies every byte onto the heap and leaves the result to the garbage collector. Letting the operating system be the cache, instead of building a worse one myself, is the kind of thing that feels like cheating the first time it works.

The format isn't finished, either. Today every token is a full 24 bytes of absolute values, and a binary search hops across the array touching a fresh page at each step, even though `mmap` already faulted in a whole 4 to 16 KB page to hand back one record. The next version I want to try is frame-of-reference encoding, the same instinct the source map uses on itself: store offsets, not absolutes. Group the tokens into fixed-length frames, keep one base value per frame, and write each token as a small delta from it. The records shrink, so the file shrinks, and far more of them fit in a single page. A lookup becomes a binary search to the right frame plus a short scan inside it, one page fault instead of a dozen. Smaller on disk and faster to read, from the same idea behind everything above: respect the shape the hardware actually pulls in.
